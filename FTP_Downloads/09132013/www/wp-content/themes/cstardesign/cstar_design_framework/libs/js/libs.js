jQuery(document).ready(function() {
	...
	var elem = jQuery("._post_type select");

	elem.each( function(){

		el_this = jQuery(this);
		jQuery( "#type_"+el_this.val() ).show().siblings().hide();

		el_this.bind('change', function(){

			el_select = jQuery(this).val();
			if( el_select ){
				jQuery("#type_"+el_select).show().siblings().hide();
			}else{
				jQuery("#post_types li").hide();
			}
		});
	...
	elem = jQuery("._link_type select");

	elem.each( function(){

		el_this = jQuery(this);
		jQuery( "#link_"+el_this.val() ).show().siblings().hide();

		el_this.bind('change', function(){

			el_select = jQuery(this).val();
			if( el_select ){
				jQuery("#link_"+el_select).show().siblings().hide();
			}else{
				jQuery("#link_types li").hide();
			}
		});
	...
	elem = jQuery("._doc_type select");

	elem.each( function(){

		el_this = jQuery(this);
		jQuery( "#doc_"+el_this.val() ).show().siblings().hide();

		el_this.bind('change', function(){

			el_select = jQuery(this).val();
			if( el_select ){
				jQuery("#doc_"+el_select).show().siblings().hide();
			}else{
				jQuery("#doc_types li").hide();
			}
		});
   ...
   elem = jQuery("._slider_link select");
   ...
   elem = jQuery("._layer1_type select");

	elem.each( function(){

		el_this = jQuery(this);
		jQuery( "#layer1_"+el_this.val() ).show().siblings().hide();

		el_this.bind('change', function(){

			el_select = jQuery(this).val();
			if( el_select ){
				jQuery("#layer1_"+el_select).show().siblings().hide();
			}else{
				jQuery("#layer1_types li").hide();
			}
		});
   ...
   elem = jQuery("._layer2_type select");

	elem.each( function(){

		el_this = jQuery(this);
		jQuery( "#layer2_"+el_this.val() ).show().siblings().hide();

		el_this.bind('change', function(){

			el_select = jQuery(this).val();
			if( el_select ){
				jQuery("#layer2_"+el_select).show().siblings().hide();
			}else{
				jQuery("#layer2_types li").hide();
			}
		});
   ...
   elem = jQuery("._layer3_type select");

	elem.each( function(){

		el_this = jQuery(this);
		jQuery( "#layer3_"+el_this.val() ).show().siblings().hide();

		el_this.bind('change', function(){

			el_select = jQuery(this).val();
			if( el_select ){
				jQuery("#layer3_"+el_select).show().siblings().hide();
			}else{
				jQuery("#layer3_types li").hide();
			}
		});

	});

   var elem = jQuery("._intro_type select");

	elem.each( function(){

		el_this = jQuery(this);

      if( el_this.val() == 'both' || el_this.val() == 'content'){
         jQuery( "#intro_type_show" ).show();
		}

		el_this.bind('change', function(){

			el_select = jQuery(this).val();
			if( el_select != 'title' && el_select != '' ){
				jQuery("#intro_type_show").show();
			}else{
				jQuery("#intro_type_show").hide();
			}
		});
...
jQuery(document).ready(function() {
   ...
   _dom = jQuery('#cstar-shortcode-generator');
   
	_dom.on('click', '.checkbox_all', function(){
		jQuery(this).parent().find("input[type=checkbox]").each(function(){
			jQuery(this).attr("checked", true);
		});
	});

	_dom.on('click', '.uncheckbox_all', function(){
		jQuery(this).parent().find("input[type=checkbox]").each(function(){
			jQuery(this).attr("checked", false);
		});
	});

	_dom.on('click', '.select_all', function(){
		jQuery(this).parent().prev().find("option[value!=]").each(function(){
			jQuery(this).attr("selected", true);
		});
	});

	_dom.on('click', '.unselect_all', function(){
		jQuery(this).parent().prev().find("option[value!=]").each(function(){
	...
	jQuery(document).ready(function() {
		jQuery('#colorpic_'+$name).ColorPicker({
			...
			onChange: function (hsb, hex, rgb) {
				...
				jQuery('.inputcolor_'+$name).attr('value', '#'+hex).trigger("bg_pic");
			}
		})
		.bind('keyup', function(){
		jQuery(this).ColorPickerSetColor(this.value);
...
jQuery(document).ready(function(){
   ...
   /* background uploader */
   jQuery(".background_uploader").each(function(){

      var elem       = jQuery(this);
      var bg         = elem.find(".upload_background_pic");
      var bg_result  = elem.find(".bg_result");

      elem.find('.background_position').bind('change focus blur keyup', function(){

         bg.css({"background-position": jQuery(this).val()});

      });


      elem.find('.background_repeat').bind('change focus blur keyup', function(){

         bg.css({"background-repeat": jQuery(this).val()});

      });

      elem.find('.background_attachment').bind('change focus blur keyup', function(){

         bg.css({"background-attachment": jQuery(this).val()});

      });


      elem.find('.bg_pic').bind('change focus blur keyup keypress bg_pic', function(){

         bg.css({"background-color": jQuery(this).val()}).trigger("bgfix");

      });

      elem.bind('change focus blur keyup bgfix', function(){
         var bg_url  = bg.css('background-image');
         var pattern  = /\"/g;
         bg_result.val( 'background-image:'+ bg_url.replace(pattern,'') +'; background-repeat:' + bg.css('background-repeat') + '; background-attachment:' + bg.css('background-attachment') + '; background-position:' + bg.css('background-position') + '; background-color:' + elem.find('.bg_pic').val() + ';' );
      });
...
jQuery(document).ready(function(){
   ...
   /* sidebar widget name checker */
   jQuery("input[name=sidebar_action]").on("click", function(){

      var error_div =  jQuery("div#sidebar_error").html('');
      var pattern = /^[a-zA-Z].{4}/g;
      var input_val = jQuery("input#add_sidebar").val();
      if(  !pattern.test(input_val) ){
         error_div.html("Error! Please write a new sidebar nicename!").hide().fadeIn();
         return false;
      }
   ...
   elem = jQuery("div#add_sidebars");
	elemClone = elem.clone(true);
	elem.remove();
	elemClone.appendTo("#widgets-left").show();

   /* background uploader */
   jQuery('.upload_bg_input').each(function(){

      jQuery(this).bind('change focus blur keypress keydown keyup framework', function(){

         $get_cur_div = '#' + jQuery(this).attr('id') + '_preview';

         if(jQuery(this).val() == ""){
            jQuery($get_cur_div).css({"background-image": "none"});
         }else{
	...
	jQuery('.upload_input').each(function()
	{
		...
		jQuery(this).bind('change focus blur framework keyup keypress keydown', function()
		{
   ...
   window.media_send_to_custom_editor = function(html) {
      ...
      jQuery('.'+elem_id).val($img).trigger('framework').trigger('bgfix');
      tb_remove();
      window.send_to_editor = window.original_send_to_editor;
   }
   
	/* window.get_id = false; */
   jQuery('div.option_upload_insert').on('click','a.set_input', function(){
