...
add_action('save_post', 'frau_feedback_options_update', 0);
add_action('admin_init', 'frau_feedback_options', 1);
add_action('admin_head', 'frau_head');
add_action('admin_menu', 'frau_themeMenu');
...
if (isset($_GET['page'])&&isset($_GET['action'])&&$_GET['action']=='reset') {
	...
	function frau_head() { ?>
		...
		<?php if (isset($_GET['page'])&&in_array($_GET['page'], array('LZSettings', 'general', 'slider', 'showroom', 'layout', 'socialbuttons', 'seo', 'menus', 'integration', '_contactform', '_freshthemes'))) { ?>
...
function frau_themeMenu() {
	global $adminsettings, $settingOptions;
	include_once (get_template_directory()."/inc/settings.php");
	$theme = wp_get_theme( );
	add_theme_page( $theme['Name'], $theme['Name'], 'manage_options', 'LZSettings', 'frau_thememanage');
	
	foreach( $adminsettings as $name=>$title ) {
		add_theme_page( 'LZSettings', $title, 'manage_options', $name, 'frau_thememanage');
	}
}

function frau_thememanage() {
	...
	<div id='lz-settings'>
		...
		<a href='?page=<?php echo $_GET['page']; ?>&action=reset' class='lz-reset'><?php _e( 'Reset', 'lizard' ); ?> <?php echo (isset($adminsettings[$_GET['page']]))?$adminsettings[$_GET['page']]:$theme['Name']; ?> <?php _e( 'settings', 'lizard' ); ?></a>
		...
		<?php foreach( $adminsettings as $name=>$title ) { ?>
			...
			<form action='' class='lz-form' method='POST'>
				...
				<div class='lz-option'>
					<?php frau_show_option($name,$option); ?>
		...
		<div class='lz-page lz-support-page'>
			<dl class='lz-support-menu'>
				...
				<dt><a href='#'>Creating contact form</a></dt>
				<dd>
...
function frau_show_option($section, $option) {
	global $settings;
	...
	switch( $option['type'] ) {
		...
		case 'social': 
			$settings[$option['values']]=frau_loadsettings( $option['values'] );?>
				<h3><?php echo $option['caption']; ?></h3>
				<p class="description"><?php echo $option['description']; ?></p>
				...
				if ( isset( $settings[$section][$option['name']] ) ) {
					...
					foreach( $settings[$section][$option['name']] as $slide) { $i++;?>
						...
						<dd>
							...
							<tr><td><?php _e( 'Image', 'lizard' ); ?>(1000px x 422px):</td><td><input type="text" name="slides[<?php echo $i; ?>][image]" value="<?php echo $slide['image']; ?>" /><span class="lz-imageupload button"><?php _e( 'Upload', 'lizard' ); ?></span></td></tr>
