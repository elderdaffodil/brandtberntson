...
if ( session_id() == '' ) 
session_start();
$settings['general']=frau_loadsettings( 'general' );
$settings['showroom']=frau_loadsettings( 'showroom' );
$settings['slider']=frau_loadsettings( 'slider' );
$settings['layout']=frau_loadsettings( 'layout' );
$settings['elements']=frau_loadsettings( 'elements' );
$settings['fonts']=frau_loadsettings( 'fonts' );
$settings['integration']=frau_loadsettings( 'integration' );
$settings['menus']=frau_loadsettings( 'menus' );
...
add_action('after_setup_theme', 'frau_custom_theme_setup');
add_action('wp_head', 'frau_custom_header_code');
add_action( 'after_setup_theme', 'frau_custom_header_setup' );
add_action( 'init', 'frau_register_theme_menus' );
add_action( 'widgets_init', 'frau_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'frau_enque' );
add_filter( 'wp_title', 'frau_wp_title', 10, 2 );
if ( ! isset( $content_width ) ) $content_width = 900;
$container=array( 'width'=>1000, 'margin-left'=>0, 'margin-right'=>0 );
add_action( 'template_redirect', 'frau_content_width' );
...
function frau_loadsettings($section) {
	global $lz_default;
	if (! $result=get_option( $section ) ) {
		include_once ( get_template_directory().'/inc/default.php' );
		$result=$lz_default[$section];
		update_option( $section, $result );
	}
	return $result;
}


function frau_custom_theme_setup() {
	global $settings;
...
function frau_register_theme_menus() {
  register_nav_menus(
    array(
...
function frau_custom_header_code() { 
	...
	<?php 
		...
		foreach ($settings['fonts'] as $key=>$value) { ?>
			<link href="http://fonts.googleapis.com/css?family=<?php echo $value; ?>&subset=latin,cyrillic" rel="stylesheet" type="text/css" />
...
function frau_custom_background_cb() {
...
function frau_theme_secondary_menu() { ?>
...
function frau_comment( $comment, $args, $depth ) {
...
function frau_custom_excerpt($args=''){
...
function frau_wp_title( $title, $sep ) {
	global $settings;
